function [h,h_1,Nfft,Nifft,doppler_coeff]=Clarke_model(fm,fs,N,E0)
% Clarke model
% function：
...
h=complex(real(hI(1:N)),-real(hQ(1:N)))/rayRMS; % h_1即为信道复包络(归一化)
h_1=E0*complex(real(hI(1:N)),-real(hQ(1:N)))/rayRMS; % h 即为信道复包络
% h_all=E0*complex(real(hI(1:N)),-real(hQ(1:N)))/rayRMS; % h 即为信道复包络
% h=h_all(1,s);